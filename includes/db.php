...
function db_resolve_config(): array
{
    ...
    $isProductionLike = db_first_non_empty(env('RAILWAY_ENVIRONMENT'), env('RAILWAY_PROJECT_ID')) !== null
        || !in_array($appEnv, ['local', 'development', 'dev'], true);

    $host = db_first_non_empty(env('DB_HOST'), env('MYSQLHOST'), env('MYSQL_HOST'));
    $port = db_first_non_empty(env('DB_PORT'), env('MYSQLPORT'), env('MYSQL_PORT'), '3306');
    $name = db_first_non_empty(env('DB_NAME'), env('MYSQLDATABASE'), env('MYSQL_DATABASE'), env('DB_DATABASE'));
    $user = db_first_non_empty(env('DB_USER'), env('MYSQLUSER'), env('MYSQL_USER'), env('DB_USERNAME'));
    $pass = db_first_non_empty(env('DB_PASS'), env('MYSQLPASSWORD'), env('MYSQL_PASSWORD'), env('DB_PASSWORD'), '');
    ...
    if ($host === null || $name === null || $user === null) {
        throw new RuntimeException(
            ...
            'o referencia correctamente las variables del servicio MySQL de Railway en el servicio web.'
        );
    }
